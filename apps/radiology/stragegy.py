...
class Order(Strategy):
    ...
    def __call__(self, request, datastore: Datastore):
        ...
        for index, image_id in enumerate(image_ids):
            ...
            self.extra_info += self.pad_string(f"""{"=>" if index == self.index else ". "} 第{index+1}份样本({image_file.name:65})：{"已标注" if label_file.exists() else "未标注"} {"标注时间:" + str(datetime.fromtimestamp(os.path.getmtime(label_file))) if label_file.exists() else ""} """)
